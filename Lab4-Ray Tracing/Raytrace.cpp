...
// Compute for shadow if hasShadow is true.
    ...
    for (const PointLightSource& lightSource : scene.ptLights) {
        ...
        if (hasShadow) {
            ...
            for (const auto& surface : scene.surfaces) {
                if (surface->shadowHit(shadowRay, DEFAULT_TMIN, L_length)) {
                    hasShadowHit = true;
                    break;
                }
            }
            ...
            if (!hasShadowHit) {
                Color phongColor = computePhongLighting(L, N, V, nearestHitRec.material, lightSource);
